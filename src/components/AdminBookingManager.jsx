...
const AdminBookingManager = ({ addToast, onBookingChanged }) => {
    ...
    const formatDateForDisplay = (utcDate, options = { timeZone: 'UTC', year: 'numeric', month: 'long', day: 'numeric'}) => {
    ...
    const handleDeleteBooking = async (bookingId, bookingStatus) => { 
        if (!window.confirm(`Delete ${bookingStatus || 'N/A'} booking? Cannot be undone.`)) return; setLoading(true);
    ...
    const handleEditInputChange = (e) => { 
        ...
        setEditForm(prev => {
            const newState = { ...prev, [name]: name === 'numberOfRooms' ? (parseInt(value, 10) || 1) : value };
    ...
    const getDisplayProgramType = (booking) => { /* ... (same as previous complete version) ... */ };
                ...
                <div className="edit-booking-form-admin">
                    <h3 className="edit-title">Edit: <span className="highlight">{selectedBookingForEdit.programTitle}</span></h3>
                     <div className="grid grid-halves">
                        <div> {/* Left Column: Non-date inputs */}

...
import { 
    getSingleDayOccupancy,
    getSpecificMonthOccupancy,
    getMonthlyOccupancyStatsForYear,
    getYearlyOccupancyStats,
    getProgramTypeBreakdownForPeriod
...
const YEAR_OPTIONS = Array.from({ length: 10 }, (_, i) => CURRENT_YEAR - i); // Increased to 10 years
...
const BookingAnalyticsDashboard = ({ addToast }) => {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        let periodStartUTC, periodEndUTC;
        try {
            if (viewMode === 'singleDay') {
                const dayUTC = convertLocalToUTCDate(selectedSingleDay);
                if (dayUTC) { setSingleDayStats(getSingleDayOccupancy(dayUTC)); periodStartUTC = dayUTC; periodEndUTC = dayUTC; }
                ...
                periodEndUTC = new Date(Date.UTC(selectedYear, selectedMonth, daysInMonth));
                ...
                periodEndUTC = new Date(Date.UTC(selectedYear, 11, 31));
            }
            if (periodStartUTC && periodEndUTC) {
                const progStats = getProgramTypeBreakdownForPeriod(periodStartUTC, periodEndUTC);
                const formattedProgData = Object.values(progStats)
                    .filter(s => s.confirmed > 0 || s.pencil > 0) 
                    .map(s => ({ name: s.label, Confirmed: s.confirmed, Pencil: s.pencil, Total: s.confirmed + s.pencil }));
        ...
        singleDayStats && (
            <>
                ...
                <div className="stats-summary-text">
                    ...
                    <p><span className="stat-label">Occupancy:</span> <span className="stat-value">{TOTAL_ROOMS > 0 ? ((singleDayStats.totalBooked / TOTAL_ROOMS) * 100).toFixed(4) : 0}%</span></p> {/* Changed to toFixed(4) */}
                </div>
            ...
            <>
                <h3 className="analytics-subtitle">Summary for {MONTH_NAMES[selectedMonth]} {selectedYear}</h3>
                 <div className="stats-summary-text">
                    ...
                    <p><span className="stat-label">Occupancy Rate:</span> <span className="stat-value">{specificMonthStats.occupancyRate.toFixed(4)}%</span></p> {/* Changed to toFixed(4) */}
                </div>
        ...
        <>
            {yearlySummaryStats && (
                ...
                <div className="yearly-stats-cards">
                    ...
                    <div className="stat-card"><h4>Overall Occupancy Rate</h4><p>{yearlySummaryStats.overallOccupancyRate.toFixed(4)}%</p></div> {/* Changed to toFixed(4) */}
                </div></>
            )}
            ...
            {yearlyMonthlyBreakdown.length > 0 ? (
                <ResponsiveContainer width="100%" height={350}>
                    <LineChart data={yearlyMonthlyBreakdown.map(m => ({name: m.month.substring(5), Confirmed: parseFloat(m.avgDailyConfirmed.toFixed(4)), Pencil: parseFloat(m.avgDailyPencil.toFixed(4))}))}> {/* Changed to toFixed(4) */}
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="name" />
                        <YAxis label={{ value: 'Avg Daily Rooms', angle: -90, position: 'insideLeft' }} allowDecimals={true}/> {/* Allow decimals on Y-Axis now */}
                        <Tooltip formatter={(value) => `${Number(value).toFixed(4)} rooms`}/> {/* Show more decimals in tooltip */}
                        <Legend />
                ...
                <ResponsiveContainer width="100%" height={350}>
                    <PieChart>
                        <Pie data={programTypeBreakdown} dataKey="Total" nameKey="name" cx="50%" cy="50%" outerRadius={120} 
                             label={({ name, percent, Confirmed, Pencil }) => `${name} (${(percent * 100).toFixed(1)}%)`} > {/* Pie label might be better with 1 decimal */}
    ...
    return (
        <div className="card analytics-dashboard">
            ...
            <div className="analytics-controls">
                ...
                {viewMode === 'singleDay' && (<div className="control-group"><label>Date:</label><DatePicker selected={selectedSingleDay} onChange={date => setSelectedSingleDay(date)} className="form-input-sm" dateFormat="MMM d, yyyy"/></div>)}
            ...
            <div className="charts-container">
                ...
                {!loading && renderProgramTypeDistribution()} 
            </div>
        </div>
    );
};
